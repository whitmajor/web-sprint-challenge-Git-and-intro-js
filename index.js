...
function getLongReviews(reviewArr) {
	for (let i = 0; i < reviewArr.length; i++) {
		...
		if (feedbackWordsArr.length > 15) {
			selectedLongReviewsArr.push(reviews[i])
		}
	}
	
	return selectedLongReviewsArr
  }
*/
...
pFor example, if removeArtist is invoked with the artists array and the number 0, it will remove Amedeo Modigliani from our dataset and return the number 19. */

function removeArtist(array,number) {
  array.splice(number,array)
...
function addArtist(array) {
  array.push({ id : 20, name : "Whitney Major", years: 1992-2022, genre: "web design", nationality: "Bahamian", bio: "I love to program.Programing is the best."
     })
 return array
}
...
function lotsOfArt(array){
  ...
  if (array[i].paintings >100){
    paintings100.push(array[i].name)
